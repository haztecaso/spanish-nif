...
- KLM cards with prefixes `K`, `L`, or `M`.
...
5. **Fallback to klm prefixes** `K`, `L`, or `M`: treat the following seven
...
- `K0867756N`: klm branch; digits `0867756`, remainder `13` → `N`.

## Formal Specification
- Input must be 9 ASCII characters after removing separators.
- Valid prefixes: digits `0-9`, klm `KLM`, or NIE `XYZ`.
- Let `letters = "TRWAGMYFPDXBNJZSQVHLCKE"`.
- For NIE prefixes, set `n = int(prefix_map[prefix] + digits)`, where
  `prefix_map = {"X": "0", "Y": "1", "Z": "2"}`.
- For klm prefixes, set `n = int(digits)` where `digits` are positions 2–8.
